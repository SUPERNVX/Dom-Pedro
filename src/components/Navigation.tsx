...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    const enterTweens = new Map<Element, gsap.core.Tween | null>();
    const leaveTweens = new Map<Element, gsap.core.Tween | null>();

    const getPathLength = (path: SVGPathElement): number =>
      path.getTotalLength();
    ...
    if (nav) {
      ScrollTrigger.create({
        ...
        onToggle: ({ isActive }) => {
          nav.classList.toggle('navigation--solid', isActive);
        },
      });
    }
    ...
    document.querySelectorAll('[data-draw-line]').forEach((container) => {
      ...
      container.addEventListener('mouseenter', () => {
        if (enterTweens.get(container)?.isActive()) return;
        leaveTweens.get(container)?.kill();

        box.innerHTML = svgVariants[nextIndex];
        const svg = box.querySelector('svg');
        if (svg) {
          svg.setAttribute('class', 'nav-draw__box-svg');
          ...
          if (path) {
            ...
            const tween = gsap.to(path, {
              ...
              duration: 0.5,
              ease: 'power2.inOut',
              onComplete: () => enterTweens.set(container, null),
            });
            enterTweens.set(container, tween);
          }
        }
      ...
      container.addEventListener('mouseleave', () => {
        ...
        const drawOut = () => {
          if (leaveTweens.get(container)?.isActive()) return;
          const length = getPathLength(path);
          ...
          const tween = gsap.to(path, {
            ...
            ease: 'power2.inOut',
            onComplete: () => {
              leaveTweens.set(container, null);
              box.innerHTML = '';
            },
          });
          leaveTweens.set(container, tween);
        };

        const enterTween = enterTweens.get(container);
        if (enterTween?.isActive()) {
          enterTween.eventCallback('onComplete', drawOut);
        } else {
          drawOut();
        }
      });
    ...
    return () => {
      enterTweens.forEach((tween) => tween?.kill());
      ...
      <div className="navigation__inner">
        ...
        <div className="navigation__links">
          {navLinks.map((link) => (
            ...
            >
              <span className="nav-draw__label">{link.label}</span>
              <div data-draw-line-box className="nav-draw__box"></div>
            </a>
          ))}
        </div>
      </div>
